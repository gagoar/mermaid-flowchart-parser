 ...
 */
function extractShapeDeclarations(line, ids) {
...
function splitOnArrows(skeleton) {
    const texts = [];
    const arrows = [];
    let lastIndex = 0;
...
export function parseFlowchart(mermaid) {
    const ids = new Set();
    const nodeOrder = [];
    const edges = [];
    const addId = (id) => {
        if (RESERVED.has(id) || ids.has(id))
            return;
        ids.add(id);
        nodeOrder.push(id);
    };
...
export function extractMermaidNodeIds(mermaid) {
